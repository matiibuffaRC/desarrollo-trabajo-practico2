...
function MonthlyLimitTracker({ monthlyLimit, setMonthlyLimit, totalExpenses = 0 }) {
    ...
    // Guardamos el límite en localStorage
    useEffect(() => {
    ...
    return (
        <div className='w-full'>
            <div onClick={() => setIsOpen(!isOpen)} className={`bg-white rounded-2xl shadow-md p-6 cursor-pointer transition-all duration-300 transform hover:shadow-lg hover:scale-[1.02] ${ isOpen ? 'rounded-b-none shadow-lg' : '' }`}>
                <div className={`transition-all duration-300 ${isOpen ? 'max-h-0 opacity-0 overflow-hidden' : 'max-h-96 opacity-100'}`}>
                    ...
                    <div>
                        <div className='flex justify-between items-center mb-2'>
                            <p className='inter text-xs text-gray-500'>
                                Progreso
                            </p>
                        ...
                        <div className='w-full bg-gray-200 rounded-full h-3 overflow-hidden'>
                            <div className={`h-full transition-all duration-500 ease-out ${isExceeded ? 'bg-red-500' : 'bg-green-500' }`} style={{ width: `${percentage}%` }} />
                        </div>
                    </div>
                        {isExceeded && (
                ...
                <div onClick={(e) => e.stopPropagation()} className={`transition-all duration-300 overflow-hidden ${isOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
                    <div className='pt-6 border-t border-gray-200'>
                        ...
                        <div className='grid grid-cols-2 gap-3 mb-5 p-3 bg-gray-50 rounded-lg'>
                            ...
                            <div>
                                <p className='inter text-xs text-gray-500 mb-1'>
                                    Gastado
                                </p>
